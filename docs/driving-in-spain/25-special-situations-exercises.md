# 25 — Special Situations Exercises / Ejercicios de Situaciones Especiales

**🏠 [Index](01-getting-started.md)** · ◀ [Prev: Special Situations](24-special-situations.md) · [Next: Exam Commands ▶](26-practical-exam-commands.md) · ↩ [Back to content: Special Situations](24-special-situations.md)

---

## ⭐ Difficulty: ⭐⭐ (Core Exam Knowledge)

---

## A. DGT-Format Questions — Preguntas tipo DGT

### Question 1

**ES:** _Al entrar en un túnel de día, ¿qué debe hacer con sus gafas de sol?_
**EN:** When entering a tunnel during the day, what should you do with your sunglasses?

- A) Ponérselas para protegerse de los reflejos / Put them on to protect from reflections
- B) Quitárselas antes de entrar / Take them off before entering
- C) No importa / It doesn't matter

### Question 2

**ES:** _Hay un incendio en un túnel. ¿Qué hace con las llaves del coche?_
**EN:** There is a fire in a tunnel. What do you do with the car keys?

- A) Llevárselas consigo / Take them with you
- B) Dejarlas puestas en el contacto / Leave them in the ignition
- C) Tirarlas al suelo / Throw them on the floor

### Question 3

**ES:** _Las luces de un paso a nivel parpadean en rojo. No ve ningún tren. ¿Qué debe hacer?_
**EN:** The lights at a level crossing flash red. You can't see a train. What should you do?

- A) Cruzar rápidamente / Cross quickly
- B) Detenerse y esperar / Stop and wait
- C) Tocar el claxon y cruzar / Sound the horn and cross

### Question 4

**ES:** _Su coche se avería en una autopista. ¿En qué orden actúa?_
**EN:** Your car breaks down on a motorway. In what order do you act?

- A) V-16, chaleco, salir, llamar al 112 / V-16, vest, exit, call 112
- B) Llamar al 112, salir, chaleco, V-16 / Call 112, exit, vest, V-16
- C) Arcén, luces de emergencia, V-16, chaleco, salir por el lado del pasajero, detrás de la barrera, llamar al 112 / Hard shoulder, hazard lights, V-16, vest, exit passenger side, behind barrier, call 112

### Question 5

**ES:** _En una retención en autopista de dos carriles, ¿cómo se crea el carril de emergencia?_
**EN:** In a traffic jam on a 2-lane motorway, how do you create the emergency corridor?

- A) Todos al carril derecho / Everyone to the right lane
- B) Carril izquierdo se abre a la izquierda, carril derecho a la derecha / Left lane opens left, right lane opens right
- C) Todos al carril izquierdo / Everyone to the left lane

### Question 6

**ES:** _¿Qué distintivo ambiental necesita un vehículo para circular por la ZBE de Valencia?_
**EN:** What environmental badge does a vehicle need to drive in Valencia's LEZ?

- A) Cualquiera — no hay restricciones / Any — there are no restrictions
- B) Depende de la normativa municipal, pero generalmente C o superior / Depends on municipal rules, but generally C or higher
- C) Solo CERO / Only CERO

### Question 7

**ES:** _Su coche se cala en las vías del tren. ¿Qué debe hacer?_
**EN:** Your car stalls on the train tracks. What should you do?

- A) Intentar arrancar el motor repetidamente / Try to restart the engine repeatedly
- B) Evacuar a todos, y correr por la vía en la dirección de donde viene el tren / Evacuate everyone and run along the track in the direction the train is coming from
- C) Esperar dentro del coche / Wait inside the car

### Question 8

**ES:** _Los VMP (patinetes eléctricos) pueden circular:_
**EN:** PMVs (electric scooters) can ride:

- A) Por la acera / On the pavement
- B) Por la calzada o el carril bici / On the road or the cycle lane
- C) Solo por zonas peatonales / Only in pedestrian zones

---

## B. True or False — Verdadero o Falso

1. \_\_\_ You must use dipped headlights in ALL tunnels, even during the day.

2. \_\_\_ In a tunnel fire, you should take your car keys with you when evacuating.

3. \_\_\_ If level crossing lights flash red but no train is visible, you may cross carefully.

4. \_\_\_ The emergency corridor requires the left lane to move left and the right lane to move right.

5. \_\_\_ Electric scooters (VMP) may ride on pavements in Spain.

6. \_\_\_ All Spanish cities with 50,000+ residents must have a ZBE by 2026.

7. \_\_\_ If your car stalls on train tracks, run perpendicular (away from the tracks at a right angle).

8. \_\_\_ Overtaking inside a single-lane tunnel is prohibited.

---

## C. Fill in the Blank — Completa las frases

**Word box:** _túnel, cruce, gafas, llaves, arcén, V-16, corredor, ZBE, ciclista, 1.5_

1. En el \_\_\_\_\_\_\_\_, use siempre luces de \_\_\_\_\_\_\_\_.

2. Quítese las \_\_\_\_\_\_\_\_ de sol antes de entrar en un túnel.

3. En caso de incendio en un túnel, deje las \_\_\_\_\_\_\_\_ en el contacto.

4. Si tiene una avería en la autopista, deténgase en el \_\_\_\_\_\_\_\_.

5. Active la baliza \_\_\_\_\_\_\_\_ y colóquela en el techo.

6. En una retención, cree un \_\_\_\_\_\_\_\_ de emergencia.

7. Para entrar en la \_\_\_\_\_\_\_\_ de Valencia, necesita el distintivo ambiental.

8. Al adelantar a un \_\_\_\_\_\_\_\_, mantenga \_\_\_\_\_\_\_\_ metros de distancia lateral.

---

## D. Translate the Phrase — Traduce la frase

1. Turn on your dipped headlights before entering the tunnel.

2. If there's a fire in the tunnel, leave the keys in the ignition and walk to the nearest exit.

3. Stop at the level crossing — the red lights are flashing.

4. My car has broken down on the motorway — I'm on the hard shoulder.

5. Create an emergency corridor: left lane moves left, right lane moves right.

6. Electric scooters must use the road or the cycle lane — not the pavement.

7. Check your environmental badge before driving into the low emission zone.

8. When overtaking a cyclist, reduce your speed by 20 km/h and give 1.5 metres.

---

## E. Situational Spanish — ¿Qué dices?

1. You've broken down on the V-30. Call 112 and describe your situation.

2. You're approaching a tunnel with your daughter in the car. Tell her what you need to do (lights, sunglasses).

3. You want to warn Pepe that he needs to check his car's environmental badge before driving into Valencia's city centre.

4. You're stuck in a traffic jam on the autopista and want to explain to your passenger why you're moving to the side of the lane.

---

---

# ✅ Answer Key — Clave de respuestas

## A. DGT-Format Questions

1. **B)** Quitárselas antes de entrar
2. **B)** Dejarlas puestas en el contacto
3. **B)** Detenerse y esperar
4. **C)** Arcén, luces de emergencia, V-16, chaleco, salir por el lado del pasajero, detrás de la barrera, llamar al 112
5. **B)** Carril izquierdo se abre a la izquierda, carril derecho a la derecha
6. **B)** Depende de la normativa municipal, pero generalmente C o superior
7. **B)** Evacuar a todos, y correr por la vía en la dirección de donde viene el tren
8. **B)** Por la calzada o el carril bici

## B. True or False

1. **V** — Dipped headlights always in tunnels.
2. **F** — Leave keys IN the ignition so firefighters can move cars.
3. **F** — STOP and wait even if no train is visible.
4. **V** — Left lane left, right lane right.
5. **F** — VMP are prohibited on pavements.
6. **V** — Cities 50K+ must have a ZBE.
7. **F** — Run ALONG the track toward the train's direction, not perpendicular.
8. **V** — Overtaking in single-lane tunnels is prohibited.

## C. Fill in the Blank

1. túnel / cruce
2. gafas
3. llaves
4. arcén
5. V-16
6. corredor
7. ZBE
8. ciclista / 1.5

## D. Translate the Phrase

1. _Encienda las luces de cruce antes de entrar en el túnel._
2. _Si hay un incendio en el túnel, deje las llaves en el contacto y camine hasta la salida más cercana._
3. _Deténgase en el paso a nivel — las luces rojas están parpadeando._
4. _Mi coche se ha averiado en la autopista — estoy en el arcén._
5. _Creen un corredor de emergencia: el carril izquierdo se abre a la izquierda y el derecho a la derecha._
6. _Los patinetes eléctricos deben circular por la calzada o el carril bici — no por la acera._
7. _Compruebe su distintivo ambiental antes de entrar en la zona de bajas emisiones._
8. _Al adelantar a un ciclista, reduzca la velocidad 20 km/h y deje 1,5 metros._

## E. Situational Spanish (suggested answers)

1. _¡Hola! Mi coche se ha averiado en la V-30, creo que estoy en el kilómetro 10, dirección Valencia. Estoy en el arcén. Tengo la baliza V-16 activada. No hay heridos pero necesito asistencia._
2. _Mira, vamos a entrar en un túnel. Voy a encender las luces de cruce y me quito las gafas de sol para ver bien dentro._
3. _Pepe, ¿tienes el distintivo ambiental en tu coche? Si no, te pueden multar en la ZBE del centro de Valencia._
4. _Me estoy moviendo al lado del carril porque tenemos que crear un corredor de emergencia. Es para que puedan pasar las ambulancias y los bomberos._

---

## Z. Case Scenarios — Casos Prácticos

### Scenario 1

**ES:** _Patinete eléctrico (VMP) sin seguro ni matrícula en febrero de 2026._
**EN:** _An e-scooter (personal mobility vehicle, PMV) with no insurance and no registration, in February 2026._

- A) No pasa nada hasta 2027 / No problem until 2027
- B) Infracción: desde el 2 enero 2026 el seguro es obligatorio; el registro desde el 30 enero 2026 / An infringement: insurance has been compulsory since 2 January 2026 and registration since 30 January 2026
- C) Solo necesita seguro si supera 25 km/h / It only needs insurance if it exceeds 25 km/h

### Scenario 2

**ES:** _Llegas a Madrid con un vehículo sin distintivo ambiental (etiqueta A)._
**EN:** _You arrive in Madrid with a vehicle that has no environmental label (label A)._

- A) Puedo circular libremente en Madrid Centro / I can drive freely in Madrid Centro
- B) Tengo restricciones importantes en la ZBE: no puedo entrar en Madrid Centro ni en Plaza Elíptica en horario regulado / I face serious restrictions in the Low Emission Zone: I can't enter Madrid Centro or Plaza Elíptica during regulated hours
- C) Pago un canon y puedo entrar sin problemas / I pay a fee and can enter without issues

### Scenario 3

**ES:** _Conduces con asistencia nivel 2 (BlueCruise/Autopilot) en una autopista española._
**EN:** _You're driving with Level 2 assistance (BlueCruise / Autopilot) on a Spanish motorway._

- A) Puedo soltar las manos en cualquier autopista / I can take my hands off on any motorway
- B) Debo mantener la atención y, salvo excepciones como las _Blue Zones_ autorizadas, las manos en el volante / I must stay attentive and — except in authorised _Blue Zones_ — keep my hands on the wheel
- C) Es ilegal usar estos sistemas en España / These systems are illegal in Spain

**Answers / Respuestas:** 1-B, 2-B, 3-B

---

**🏠 [Index](01-getting-started.md)** · ◀ [Prev: Special Situations](24-special-situations.md) · [Next: Exam Commands ▶](26-practical-exam-commands.md) · ↩ [Back to content: Special Situations](24-special-situations.md)
